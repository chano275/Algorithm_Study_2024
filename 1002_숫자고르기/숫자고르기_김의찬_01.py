...
def DFS(store_start, start, visited):
    global ans  # 전역 변수 ans를 사용
    # 시작 노드와 저장된 노드가 같으면 사이클을 발견한 것
    if store_start == start:
        ans.update(visited)  # 방문한 노드를 결과 집합에 추가
        return
    # 이미 방문한 노드라면 종료
    if start in visited:
        return
...
for now in range(1, N + 1):  # 각 노드에 대해
    temp_set = set()  # 현재 DFS에 사용될 방문 집합
    if visited[now]:  # 이미 방문한 노드면 건너뛰기
        continue
